...
namespace TuneOrBuy.Services.Cars
{
    public class CarService : ICarService
    {
        ...
        {
            var car = await context.Cars.FirstOrDefaultAsync(c => c.Id == Guid.Parse(carId));
        ...
        {
            var car = await context.Cars.FirstOrDefaultAsync(c => c.Id.ToString().ToLower() == carId.ToLower());
        ...
        public async Task EditCarAsync(string id, string manufacturer, string brand, string bodyType, string vin, string fuel, int horsePower,
                                       ...
                                       string description, bool serviceHistory)
        {
            var car = await context.Cars.FirstOrDefaultAsync(c => c.Id.ToString().ToLower() == id.ToLower());
        ...
        public async Task<Tuple<bool, Buyer, Car>> ContainsCarAsync(string carId, string userId)
        {
            var car = await context.Cars
                                   .FirstOrDefaultAsync(c => c.Id.ToString().ToLower() == carId.ToLower());
